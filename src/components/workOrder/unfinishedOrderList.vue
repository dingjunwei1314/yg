<template>
    <div class="unfinished-order">
        ...
        <div>
            <el-button size="mini" type="primary" v-if="cp('--/work/abnormal/finish/export')" @click="exportExel(1)">导出当前列表</el-button>

            <el-button size="mini" type="primary" v-if="cp('--/work/a/finish/exports')" @click="exportExel(2)">导出全部列表</el-button>
            <el-table 
                :data="tableData"
                ...
                <el-table-column
                    ...
                    <template slot-scope="scope">
                        <el-button size="mini" type="primary" v-if="cp('--/work/abnormal/finish/list')" @click="view(scope.row.id)">查看</el-button>
                    </template>
                </el-table-column>
                <el-table-column
                    label="ID"
    ...
    export default {
        ...
        methods: {
            ...
            exportExel(type){
                let url = '',req;
                if(type == 1){
                    url = '/work/abnormal/finish/export';
                    req = {role_id:this.role_id,where:this.where}
                }else{
                    url = '/work/abnormal/finish/exports';
                ...
                this.$http.post(url,req,{type:'payload'}).then((response) => {
                    ...
                    if(res.code == 200){
                        if(res.data.data.length===0){
                            ...
                            let props=['city_name','order_num','status','task_value','brand','license_tag','full_name','phone_num','user_company','create_time','receive_time'];
                            exportExcel('未完成工单列表',titleSet,props,res.data.data);
                        }
                    }else{
                        message(this,res.message,'error')
                    }
